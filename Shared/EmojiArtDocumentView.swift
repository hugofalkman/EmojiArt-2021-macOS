...
struct EmojiArtDocumentView: View {
    ...
    private func pasteBackground() {
        ...
        if let imageData = Pasteboard.imageData {
            ...
            alertToShow = IdentifiableAlert(
                ...
                message: "No image found on the pasteboard"
            )
        }
    }
    ...
    private func showFetchFailedAlert(_ url: URL) {
    ...
    @SceneStorage("EmojiArtDocumentView.statePanOffset")
    private var statePanOffset = CGSize.zero
    
    @GestureState private var gesturePanOffset = CGSize.zero
    private var panOffset: CGSize {
        (statePanOffset + gesturePanOffset) * zoomScale
    }
    
    private func panGesture() -> some Gesture {
        DragGesture()
            .updating($gesturePanOffset) { latestDragValue, gesturePanOffset, _ in
                gesturePanOffset = latestDragValue.translation / zoomScale
            }
            .onEnded { finalDragValue in
                statePanOffset = statePanOffset + (finalDragValue.translation / zoomScale)
            }
    }
    
    @SceneStorage("EmojiArtDocumentView.stateZoomScale")
    private var stateZoomScale: CGFloat = 1
    
    @GestureState private var gestureZoomScale: CGFloat = 1
    private var zoomScale: CGFloat {
        stateZoomScale * gestureZoomScale
    }
    
    private func zoomGesture() -> some Gesture {
        MagnificationGesture()
            .updating($gestureZoomScale) { latestScale, gestureZoomScale, _ in
                gestureZoomScale = latestScale
            }
            .onEnded { scaleAtEnd in
                stateZoomScale *= scaleAtEnd
            }
    }
    ...
    private func zoomToFit(_ image: UIImage?, in size: CGSize) {
        if let image = image, image.size.width > 0, image.size.height > 0,
           size.width > 0, size.height > 0 {
            ...
            statePanOffset = .zero
            stateZoomScale = min (hZoom, vZoom)
        }
    }
}
